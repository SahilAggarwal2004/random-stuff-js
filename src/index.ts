...
const randomNumber = (min: number = 0, max: number = Number.MAX_SAFE_INTEGER): number => min + Math.floor(Math.random() * max)

const maximumNumber = (array: any[]) => Math.max(...array)
...
export { randomNumber, maximumNumber, randomElement, randomAdjective, randomAnimal, randomName, generateOTP }